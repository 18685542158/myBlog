<template>
    ...
    <!-- 大盒子 -->
    <div class="bg" :class="{ 'active': useMusic.light.isLight }">
        <!-- 盒子布局 -->
                    ...
                    <div class="def-nav-item" v-for="(item, index) in defNavData" :index="index" :key="item.id">
                        ...
                        <ul>
                            <li v-for="(childItem, index) in item.child" :key="childItem.path"
                                ...
                                <span :class="`iconfont icon-${childItem.icon}`"></span>{{ childItem.title }}
                            </li>
                        </ul>
                    </div>
                                ...
                                <div class="toQQ-line">
                                    <div class="imginfo-contentnav">
                                        <div class="toQQ" @click="toQQmusic"><span
                                                class="iconfont icon-tiaozhuan imginfo-contentnav-font"></span>toQQmusic
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="footer" style="width: 100%;height: 80px;">
                    ...
                    <div class="foot-body">
                        <div class="f-left">
                            ...
                            <div class="f-info">
                                <span class="name" :title="songData.name"
                                    @click="router.push({ name: 'SongDetail', params: { songmid: songData.songmid } })">{{
                                        songData.name }}</span>
                                <!-- <span class="artist" :title="songData.artist">{{ songData.artist }}</span> -->
                                <div class="outer-artist">
                                    <span class="artist" v-for="(childItem, childIndex) in songData.artist"
                                        :key="childIndex"
                        ...
                        <div class="f-center">
                            ...
                            <div class="audioModel" @click="changePlayModel">
                                ...
                                <span v-else-if="playModel == 'random'" class="iconfont icon-random"
                                    title="随机播放"></span>
                        ...
                        <div class="f-right">
                            <div class="f-control">
                                <span class="iconfont icon-xinxi"
                                    @click="router.push({ name: 'SongDetail', params: { songmid: songData.songmid } })"></span>
...
<script setup>
    ...
    import SetCookie from '../components/SetCookie-QR.vue';
    ...
    const loadSong = async (songmid) => {
        ...
        musicPlayer.loadSong(songData).then(() => {
            ...
            console.log(err);
            console.log('歌曲加载失败');
            alert('由于版权等种种原因，无法播放该音乐：' + songData.name)
        })
    ...
    watch(songData.songmid, (newValue) => {
