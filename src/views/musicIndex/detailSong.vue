    ...
    <div class="box">
        ...
        <div class="select" v-if="!loading">
            <ul>
                <li v-for="(item, index) in selectArr">
                    <div class="selItem" @click="() => {
                        selItem = index
                    }" :class="selItem == index ? 'active' : ''" v-if="item.mark">
                        <span>{{ item.name }}</span>
                    </div>
                </li>
            </ul>
        ...
        <div class="album" v-else-if="!loading && selItem == 1">
            ...
            <div class="left">
                <span>专辑：</span>
                <span
                    @click="router.push({ name: 'AlbumDetail', params: { albummid: songData.track_info.album.mid } })">
