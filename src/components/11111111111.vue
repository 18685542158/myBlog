...
const fanhui = () => {
    // router.push('/home')
    console.log('=============');
    console.log('这是pinia里面的songPlayList:');
...
const lastSong = debounce(async () => {
    isplay.value = false
    // 更新hisIndex
    hisIndex = hisList.findIndex(item => item.songmid == songmid.value)
    let lastSongmid = ''
    if (hisIndex == 0) {
        // 处理历史播放的上一曲，假如列表里面没有上一曲，就需要随机一首歌单里面的歌曲=========
        lastSongmid = nextSongmid.value
    } else {
        lastSongmid = hisList[hisIndex - 1].songmid
    }
...
const addHisList = (songData) => {
    ...
    if (index != -1 && index > hisIndex) {
        ...
        if (hisIndex == hisList.length - 1) {
            hisList.push(songData)
        } else {
            hisList.splice(hisIndex + 1, 0, songData)
        }
...
const nextSong = debounce(async () => {
    ...
    hisIndex = hisList.findIndex(item => item.songmid == songmid.value)
    let nextSongMid = ''
    if (hisIndex != -1 && hisIndex < hisList.length - 1) {        // 如果是历史播放列表产生的下一曲
        nextSongMid = hisList[hisIndex + 1].songmid
    } else {                                    // 如果是由播放器的下一曲按钮在历史播放列表里面已经到头了而产生的下一曲
